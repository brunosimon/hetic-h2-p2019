<?php
    ...
    foreach($entries as $_entry)
    {
        // Parse folder name
        $matches = array();
        preg_match_all("/(trim_[0-9])-(.*)-cours\_(.*)-(.*)/",$_entry,$matches);
        $file            = new stdClass();
        $file->url       = '../cours/'.$_entry;
        $file->trim      = str_replace('trim_', '', $matches[1][0]);
        $file->date      = str_replace('_', '/', $matches[2][0]);
        $file->num       = $matches[3][0];
        $file->title     = $matches[4][0];
...
<head>
    <meta charset="UTF-8">
    <title>Développement Web - P2019 - Bruno Simon</title>
...
<body>
    <nav class="navbar navbar-inverse">
        <div class="container-fluid">
            <a class="navbar-brand" href="#">P2019 - Dév<span class="hidden-xs">eloppement</span> Web</a>
            <ul class="nav navbar-nav navbar-right">
                <li><a href="https://github.com/brunosimon/hetic-p2019" target="_blank">GitHub</a></li>
            </ul>
        </div>
    </nav>
    <div class="container main">
        ...
        <table class="table table-striped">
            <tr>
                ...
                <th class="trim">Trim<span class="hidden-xs">estre</span></th>
            ...
            <?php foreach($files as $_file): ?>
                    <tr>
                        <td class="date hidden-xs"><?php echo $_file->date; ?></td>
                        <td class="trim"><?php echo $_file->trim; ?></td>
